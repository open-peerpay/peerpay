...
登录管理台，在“通知设置”页面填写飞书群自定义机器人 Webhook 并开启飞书通知。配置保存在数据库中，保存后立即生效，不需要设置环境变量或重启服务。

每次成功创建新订单以及订单支付成功时，系统都会异步发送一张消息卡片。原生 `/api/orders` 和 EasyPay 接口创建的订单都会通知；安卓到账匹配或后台手动确认支付后会发送支付成功通知。Webhook 发送失败不会影响订单创建、支付状态更新或商户回调，结果会记录到系统日志。

卡片包含 PeerPay 订单号、商户订单号、实付金额、原始订单金额、支付方式、收款账号、商品主题以及创建、过期或支付时间。未配置或未启用时不会发送通知。
